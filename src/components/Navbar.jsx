    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        <Link to="/" className='flex items-center gap-2' onClick={() =>{ setActive(""); window.scrollTo(0,0);}}>
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer flex'>Mateusz &nbsp;<span className='sm:block hidden'>| &nbsp;mlaskowski7</span></p>
        </Link>
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          ...
          <div className={`${!toggle ? 'hidden':'flex'} p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
            ...
            {navLinks.map((Link) =>(
              <li key={Link.id} className={`${active === Link.title ? "text-white": "text-secondary"} font-poppings font-medium cursor-pointer text-[16px]`} onClick={() => {
